package dtu.group5.backend.service.activity.edit_activity.edit_activity_project;

import dtu.group5.backend.model.BaseActivity;
import dtu.group5.backend.model.ProjectActivity;
import dtu.group5.backend.service.activity.edit_activity.ActivityFieldEditor;

import java.util.Optional;


public class StartWeekEditor implements ActivityFieldEditor {
    @Override
    public boolean supports(String fieldName) {
        return "startweek".equalsIgnoreCase(fieldName);
    }

    @Override
    public Optional<String> edit(BaseActivity activity, Object value) {
        if (!(activity instanceof ProjectActivity projectActivity))
            return Optional.of("Activity is not a project activity");

        if (value instanceof Integer newStartWeek) {
            if(newStartWeek < 0 || newStartWeek >= 52) {
                return Optional.of("Start week must be between 1 and 52");
            }
            if (projectActivity.getStartYear() > projectActivity.getEndYear() || (projectActivity.getStartYear() == projectActivity.getEndYear() && newStartWeek > projectActivity.getEndWeekNumber())) {
                return Optional.of("Start time must be equal to or before end time");
            }


            projectActivity.setStartWeekNumber(newStartWeek);
        } else {
            return Optional.of("Invalid value type for start week. Expected an Integer");
        }
        return Optional.empty();
    }
}
